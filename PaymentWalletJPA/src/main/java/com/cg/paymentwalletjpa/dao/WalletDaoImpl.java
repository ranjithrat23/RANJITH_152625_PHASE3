...
public class WalletDaoImpl implements IWalletDao {
	...
	public void fundTransfer(String userIdSender, String userIdReceiver, double amount) throws WalletException {
		...
		wallet.setBalance(customer.getWallet().getBalance() + amount);
		LocalDateTime time=LocalDateTime.now();
		String trans=customer.getWallet().getTransaction().concat(amount+" Received from "+userIdSender+" at "+time+" aaa");
		wallet.setTransaction(trans);
		customer.setWallet(wallet);
		manager.merge(customer);
		...
		String trans2=customer2.getWallet().getTransaction().concat(amount+" Sent to "+userIdReceiver+" at "+time+" aaa");
